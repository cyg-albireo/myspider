...
import itnews_spider

class Storage(itnews_spider.Spider):
    #多进程执行
    ...
    def multi_threading_execute(self, start, offset, single_num):
        ...
        threads = [threading.Thread(target=self.insert_datas, args=(start+offset+x*single_num, start+offset+(x+1)*single_num)) for x in range(5)]
        for i in range(5):
            threads[i].start()
        for i in range(5):
            threads[i].join()
    ...
    def insert_datas(self, start_page, end_page):
        ...
        for url in url_lists:
            ...
            for info in infos:
                ...
                try:
                    mysql_insert = "insert into itnews_test(title, link, posttime, tag, view, comment) values('%s', '%s', '%s', '%s', '%s', '%s')" % (info['title'], info['link'], info['posttime'], info['tag'], info['view'], info['comment'])
